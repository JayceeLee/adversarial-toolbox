...
def Discriminator(inputs):
    ...
    if base_d == True:
        ...
        output = MaxPool(output, [1, 1, 2, 2], [1, 1, 2, 2], 'SAME')
        output = lib.ops.conv2d.Conv2D('Discriminator.1', 3, 48, 3, output, stride=1)
        output = LeakyReLU(output)
        output = lib.ops.conv2d.Conv2D('Discriminator.2', 48, 48, 3, output, stride=1)
        output = LeakyReLU(output)
        #output = MaxPool(output, [1, 1, 2, 2], [1, 1, 2, 2], 'SAME')
        output = tf.nn.dropout(output, .25)

        output = lib.ops.conv2d.Conv2D('Discriminator.3', 48, 96, 3, output, stride=1)
        output = LeakyReLU(output)
        output = lib.ops.conv2d.Conv2D('Discriminator.4', 96, 96, 3, output, stride=1)
        output = LeakyReLU(output)
        #output = MaxPool(output, [1, 1, 2, 2], [1, 1, 2, 2], 'SAME')
        output = tf.nn.dropout(output, .25)

        output = lib.ops.conv2d.Conv2D('Discriminator.5', 96, 192, 3, output, stride=1)
        output = LeakyReLU(output)
        output = lib.ops.conv2d.Conv2D('Discriminator.6', 192, 192, 3, output, stride=1)
        output = LeakyReLU(output)
        #output = MaxPool(output, [1, 1, 2, 2], [1, 1, 2, 2], 'SAME')
        output = tf.nn.dropout(output, .25)

        output = tf.reshape(output, [-1, 4*4*4*48])
        output = lib.ops.linear.Linear('Discriminator.Output', 4*4*4*48, 1, output)
...
def generate_image(frame, true_dist):
    ...
    lib.save_images.save_images(samples.reshape((128, 3, 32, 32)), 'samples_{}'.format(frame), individual=False)
...
x = np.load(os.getcwd()+'/../toolkit/npy/generic2_fgsm.npy')
def adv_generator(x, batch_size):
...
def adv_load(x, batch_size):
    ...
    return (adv_generator(x[:int(x.shape[0]*.8)], batch_size), adv_generator(x[int(x.shape[0]*.2):], batch_size))

train_gen, dev_gen = adv_load(x, BATCH_SIZE)
