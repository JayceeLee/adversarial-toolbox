...
import tflib.medium_imagenet
import tflib.plot
import argparse
import generators
import discriminators

DATA_DIR = '/home/neale/repos/adversarial-toolbox/images/imagenet12'
...
with tf.Session() as session:
    ...
    real_data_int = tf.placeholder(tf.int32, shape=[BATCH_SIZE, 3, 128, 128])
    ...
    def generate_image(iteration, random=False):
        ...
        samples = samples.reshape((BATCH_SIZE, 3, 128, 128))
    ...
    def get_inception_score():
        ...
        samples = samples.reshape((-1, 3, 128, 128)).transpose(0, 2, 3, 1)
        return lib.inception_score.get_inception_score(list(samples))

    # Dataset iterator
    train_gen, dev_gen = lib.medium_imagenet.load(BATCH_SIZE, data_dir=DATA_DIR)
