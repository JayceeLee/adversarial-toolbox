...
def scale_data(args, _data):
    new_dim = tuple((s//args.downsample for s in _data.shape[1:3]))
    scale = transforms.Compose([transforms.ToPILImage(),
        transforms.Resize(new_dim),
        transforms.ToTensor(),
        transforms.Normalize(
        mean=[0.485, 0.456, 0.406],
        std=[0.229, 0.224, 0.225])]
        )
    _data = _data.reshape(args.batch_size, *_data.shape[1:]).transpose(0, 2, 3, 1)
    downsampled_data = torch.stack([scale(item) for item in _data]).cuda(0)
