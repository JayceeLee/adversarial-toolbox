...
adv_dir = '../../images/adversarials/deepfool/imagenet/symmetric/resnet_test0'
real_images = '../../images/imagenet12/recovery_set_all'


def predictions(model, x, proba=False, topk=0):
...
def load_recovery_model(bilateral=False):
    ...
    x = AveragePooling2D(pool_size=(3, 3), strides=(1, 1), padding='same')(inputs)
...
if __name__ == '__main__':
    ...
    preds2 = predictions(model, adv_good)
    top10 = predictions(model, real_good, proba=False, topk=10)
    top5 = predictions(model, real_good, proba=False, topk=5)
    ...
    for f in filters:
        for s in sigma_size:
            ...
            #adv_avg = predictions(model_avg, adv_good, proba=True)
            adv_bilateral = bilateral(adv_good, f, s, s)
            preds_rec = predictions(model, adv_bilateral)
            preds_rec_10 = predictions(model, adv_bilateral, topk=10)
