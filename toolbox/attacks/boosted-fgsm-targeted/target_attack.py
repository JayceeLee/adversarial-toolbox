...
def load_target_class(input_dir):
    ...
    print(input_dir)
    path = os.path.join(input_dir, 'target_class.csv')
    print(path)
    # with tf.gfile.Open(path) as f:
    #     a = {row[0]: int(row[1]) for row in csv.reader(f) if len(row) >= 2}
    #     print(a)
    with open(path, 'rb') as f:
        freader = csv.reader(f, delimiter=' ')
        a = {row[0]: int(row[1]) for row in freader}
        return a
...
def main(_):
  ...
  print(len(all_images_target_class), " targets")
  ...
  with tf.Graph().as_default():
    ...
    with tf.Session() as sess:
      ...
      for filenames, images in load_images(FLAGS.input_dir, batch_shape):
        target_list = [all_images_target_class[n] for n in filenames]
        target_class_for_batch = (target_list + [0] * (FLAGS.batch_size - len(filenames)))
