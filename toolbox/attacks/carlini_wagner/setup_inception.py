...
ils_val_dir = '/home/neale/repos/adversarial-toolbox/images/imagenet12/val_labels/'
...
class InceptionModel:
    ...
    def predict(self, image, pred=False, sess=None):
        if pred is True:
            x = image.astype(np.uint8)
            img = tf.placeholder(tf.uint8, (299, 299, 3))
            softmax_tensor = tf.import_graph_def(
                sess.graph.as_graph_def(),
                input_map={'DecodeJpeg:0': tf.reshape(x, ((299, 299, 3)))},
                return_elements=['softmax/logits:0'])

            predictions = sess.run(softmax_tensor, {img: x})
            predictions = np.squeeze(predictions)
            node_lookup = NodeLookup()
            top_k = predictions.argsort()[-FLAGS.num_top_predictions:][::-1]
            for node_id in top_k:
        ...
        scaled = (0.5 + tf.reshape(image, ((299, 299, 3)))) * 255
        softmax_tensor = tf.import_graph_def(
...
def readimg(ff):
    f = ils_val_dir+ff
    img = np.array(scipy.misc.imresize(scipy.misc.imread(f),
                   (299, 299)), dtype=np.float32) / 255 - .5
...
class ImageNet:
    def __init__(self):
        ...
        pool = Pool(8)
        r = pool.map(readimg, os.listdir(ils_val_dir)[:100])
        print os.listdir(ils_val_dir)[:100]
